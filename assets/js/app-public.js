          ...
          <button class="result-card result-card-list" data-company-link="${item.slug}" data-exp-id="${item.experienceId}" style="animation-delay:${i*50}ms">
            ...
            <div class="result-overlay result-overlay-list">
              <div class="result-content result-content-list">
                ...
                <h3>${escapeHtml(item.companyName)}${item.planBadge==='featured'?'<span class="result-plan-badge result-badge-featured">★ Destacado</span>':item.planBadge==='verified'?'<span class="result-plan-badge result-badge-verified">✓ Verificado</span>':''}</h3>
...
function renderCustomerBookings(){const account=requireCustomerSession('/auth?mode=login&redirect=%2Fmy-bookings');if(!account)return;app.innerHTML=shell('Mis reservas',`<section class="panel"><div class="panel-head"><h2>Historial de reservas</h2></div><div class="xp-loading"><div class="xp-spinner"></div><p>Cargando reservas…</p></div></section>`,'public');fetchCustomerBookings().then(rows=>{
  app.innerHTML=shell('Mis reservas',`<section class="panel"><div class="panel-head between"><h2>Historial de reservas</h2><a class="status-pill" href="#/account">Mi cuenta</a></div>${rows.length?`<div class="table-wrap"><table><thead><tr><th>Empresa</th><th>Oferta</th><th>Estado</th><th>Total</th><th>Fecha</th><th></th></tr></thead><tbody>${rows.map(r=>`<tr><td>${escapeHtml(r.companyName)}</td><td>${escapeHtml(r.offerName)}</td><td>${escapeHtml(r.status||'pending')}</td><td>${formatEuro(r.totalPrice)}</td><td>${new Date(r.createdAt).toLocaleString('es-ES')}</td><td><button class="booking-chat-btn" data-company-id="${escapeHtml(r.companyId)}" data-company-name="${escapeHtml(r.companyName)}" title="Contactar con la empresa"><svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></button></td></tr>`).join('')}</tbody></table></div>`:'<p class="muted">Todavía no tienes reservas registradas.</p>'}</section>`,'public');
  // Listeners botones chat por reserva
  app.querySelectorAll('.booking-chat-btn').forEach(btn=>{
    btn.addEventListener('click',()=>{
      const account=getCustomerAccount();
      if(!account){setHashRoute('/auth?mode=login');return;}
      openCustomerChat(btn.dataset.companyId, btn.dataset.companyName);
    });
  });
}).catch(error=>{app.innerHTML=shell('Mis reservas',`<section class="panel"><p>${escapeHtml(mapAuthError(error))}</p></section>`,'public');});}
...
function renderCart(){
  ...
  app.querySelector('#cart-register-form')?.addEventListener('submit',async e=>{
    ...
    try{
      await registerCustomerAccount({displayName:f.displayName.value,email:f.email.value,password:f.password.value,phone:f.phone?.value||'',marketingOptIn:f.marketingOptIn.checked,ageRange:null});
